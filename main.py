...
def main():
    ...
    df["log_level_num"] = (
        df["log_level"]
        .replace("info", np.int64(0.0))
        .replace("error", np.int64(1.0))
        .replace("warn", np.int64(2.0))
    )
    ...
    for i in range(x.shape[1] - 1):
        lst = df.iloc[
            i, 5
        ]  # using "lst" as a variable name is bad practice. We should change this Rob with something meaningful.
